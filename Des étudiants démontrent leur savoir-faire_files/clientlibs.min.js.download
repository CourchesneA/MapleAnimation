...
}u.click(function(){var x=a(this);
var i=p.filter(x.attr("href"));
var w=i.prevAll().length;
var y=(w>l||h!==null)?1:-1;
if(!x.is(".cq-carousel-active")){u.removeClass("cq-carousel-active");
x.addClass("cq-carousel-active");
if(q.is(":animated")){q.stop(true,true);
m.stop(true,true)
}if(d){i.css({left:y*e}).animate({left:0,opacity:1},k);
q.animate({left:-y*e,opacity:0},k)
}else{if(a.browser.version>6){i.css({left:y*e,opacity:1}).animate({left:0},k);
q.animate({left:-y*e},k)
}else{i.fadeIn();
q.fadeOut()
}}m=q;
q=i;
