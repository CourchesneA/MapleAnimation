...
},getForm:function(i){return this._forms[i]
...
g.registerHelper("pages",function(m,k){var n=m;
if(n.hasOwnProperty("selectedPage")&&n.hasOwnProperty("totalPages")&&n.hasOwnProperty("pageSize")&&n.hasOwnProperty("basePageURL")){var j="";
if(n.totalPages<=1){return j
}var h=Math.abs(n.pageSize);
var o=(n.pageSize<0)?"-":"";
for(var l=1;
l<=n.totalPages;
l++){n.pageNumber=l;
n.currentPageUrl=n.basePageURL+"."+((l-1)*h)+"."+o+h+".html";
n.currentPage=l==n.selectedPage;
n.suffix=((l-1)*h)+"."+o+h;
j+=k.fn(n)
}return j
}else{return""
...
k=k.replace(/\bdd\b/gi,"DD");
k=k.replace(/\bd\b/gi,"D");
k=k.replace(/\bEEEE\b/gi,"dddd");
f.locale(e.shared.I18n.getLocale());
return new g.SafeString(f(h).format(k))
});
...
var m;
if(k.get("resourceType")==="social/journal/components/hbs/journal"||k.get("resourceType")==="social/calendar/components/hbs/calendar"||(k.get("resourceType")==="social/calendar/components/hbs/event"||k.get("resourceType")==="social/journal/components/hbs/entry_topic")&&j[0].dataset.rteType!=="comment"){m=s.indexOf("oembed");
if(m===-1){l.toolbar[0].items.push("oembed")
}if(l.extraPlugins===undefined){l.extraPlugins="oembed"
}else{if(l.extraPlugins.length>0&&l.extraPlugins.indexOf("oembed")===-1){l.extraPlugins=l.extraPlugins.concat(",oembed")
}}}else{m=s.indexOf("oembed");
if(m>0){s.splice(m,1)
...
var m=k;
i.autocomplete({source:function(p,n){var o=m+".social.0.20.json?search="+d(l).val()+"&showUsers=true";
$.get(o,function(q){var r=q.items;
i.data("lastQueryResult",r);
n(r)
})
},minLength:1,change:function(n,o){j.model.set("composedForValid",j.validateUser(i.val()))
